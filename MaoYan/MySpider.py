...
class MYSpider(object):
    ...
    def __init__(self):
        ...
        # 建立字体文件对象
        base_font = TTFont("base_.woff")
        base_glyph = base_font["glyf"]

        # 组装出基础映射字典
        self.base_num_glyph_map = {
            0: base_glyph["uniE633"],
            1: base_glyph["uniE51E"],
            2: base_glyph["uniECB1"],
            3: base_glyph["uniF014"],
            4: base_glyph["uniF76B"],
            5: base_glyph["uniF0AD"],
            6: base_glyph["uniF509"],
            7: base_glyph["uniF8A4"],
            8: base_glyph["uniF4B3"],
            9: base_glyph["uniE70D"],
        }
        print(base_glyph["uniE633"])
        print(base_glyph["uniE51E"])
        print(base_glyph["uniECB1"])
        print(base_glyph["uniF014"])
        self.new_num_glyph_map = {}
        self.code_list = []


    def decryption_font(self, font_url):
        """字体解密保存"""

        response_font = requests.get(font_url)

        # 打开文件保存字体文件
        with open('new_.woff', 'wb') as f:
    ...
    def font_map(self):
        """对字体生成新的映射"""
        # 生成当前字体文件的对象
        font = TTFont('new_.woff')
        font.saveXML('new_.xml')  # 将ttf文件生成xml文件并保存到本地
        # 获取当前字体的code和name映射
        ...
        for code, name in code_name_cmap2.items():
            ...
            # 将默认映射替换成想要的样式
            code = str(hex(code)).replace("0x", "")
            self.code_list.append(code)
            # print(code, num)
            # 将新字体的映射组成字典保存
            self.new_num_glyph_map[code] = num
        print(self.new_num_glyph_map)
